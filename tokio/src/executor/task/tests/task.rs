...
fn create_complete_drop() {
    ...
    let mock = &mut || Some(NonNull::from(&mock).cast::<()>());
...
fn create_wake_drop() {
    ...
    assert_none!(task.run(&mut || Some(NonNull::from(&mock).cast::<()>())));
...
fn complete_on_second_schedule_obj() {
    ...
    // Task is returned
    let task = assert_some!(task.run(&mut || Some(NonNull::from(&mock2).cast::<()>())));

    assert_none!(task.run(&mut || Some(NonNull::from(&mock1).cast::<()>())));
...
fn join_task_immediate_drop_handle() {
    ...
    assert!(task.run(&mut || Some(NonNull::from(&mock).cast::<()>())).is_none());
...
fn join_task_complete_later() {
    ...
    let task = assert_some!(task.run(&mut || Some(NonNull::from(&mock).cast::<()>())));
...
fn complete_second_schedule_obj_before_join() {
    ...
    assert_none!(task.run(&mut || Some(NonNull::from(&mock2).cast::<()>())));
...
fn shutdown_from_list_after_complete() {
    ...
    assert_pending!(handle.poll());
    let task = assert_some!(task.run(&mut || Some(NonNull::from(&m1).cast::<()>())));
    assert_none!(task.run(&mut || Some(NonNull::from(&m2).cast::<()>())));
    assert!(did_drop.did_drop_future());
    assert!(handle.is_woken());
