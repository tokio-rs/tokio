...
trait LinkedListBase<L: Link> {
    ...
    unsafe fn remove(&mut self, node: NonNull<L::Target>) -> Option<L::Handle> {
        if let Some(prev) = L::pointers(node).as_ref().get_prev() {
            ...
            self.set_head(L::pointers(node).as_ref().get_next());
        }
        ...
        if let Some(next) = L::pointers(node).as_ref().get_next() {
            ...
            self.set_tail(L::pointers(node).as_ref().get_prev());
        }
...
impl<L: Link> LinkedListBase<L> for LinkedList<L, L::Target> {
    fn head(&mut self) -> Option<NonNull<<L as Link>::Target>> {
        self.head
    }

    fn tail(&mut self) -> Option<NonNull<<L as Link>::Target>> {
        self.tail
    }

    fn set_head(&mut self, node: Option<NonNull<<L as Link>::Target>>) {
        self.head = node;
    }

    fn set_tail(&mut self, node: Option<NonNull<<L as Link>::Target>>) {
        self.tail = node;
    }
}
        ...
        pub(crate) unsafe fn push_front(&self, val: L::Handle) {
            ...
            if old_head.is_null() {
                ...
                *self.tail.get() = Some(new_head);
                ...
                L::pointers(NonNull::new_unchecked(old_head)).as_mut().set_prev(Some(new_head));
            }
        }
