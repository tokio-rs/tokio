...
pub(crate) struct ShardedInject<T: 'static> {
    ...
    /// Round-robin counters used in place of the thread-local RNG (loom
    /// requires each execution to be deterministic).
    #[cfg(loom)]
...
impl<T: 'static> ShardedInject<T> {
    ...
    #[cfg(loom)]
    fn push_shard(&self) -> usize {
        self.push_index.fetch_add(1, Relaxed) % self.shards.len()
    }
    ...
    #[cfg(loom)]
    fn scan_start(&self) -> usize {
        self.scan_index.fetch_add(1, Relaxed) % self.shards.len()
    }
