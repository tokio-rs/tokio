...
impl Builder {
    ...
    cfg_event_loop! {
        ...
        /// JavaScript host event loop itself: the runtime schedules its own
        /// drives on the host loop, so the program spawns and returns to the
        /// host, and the tasks run from there. The event loop keeps the
        /// Emscripten runtime alive while it has tasks.
        ///
        /// # Panics
        ///
        /// This will panic if the runtime is configured with [`new_multi_thread()`].
