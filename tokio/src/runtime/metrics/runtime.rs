...
impl RuntimeMetrics {
    ...
    pub fn remote_schedule_count(&self) -> u64 {
        self.handle.spawner.scheduler_metrics().remote_schedule_count.load(Relaxed)
    }

    /// Returns the total number of times this worker thread has parked.
    pub fn worker_park_count(&self, worker: usize) -> u64 {
        self.handle.spawner.worker_metrics(worker).park_count.load(Relaxed)
    }
    ...
    pub fn worker_total_busy_duration(&self, worker: usize) -> Duration {
        let nanos = self.handle.spawner.worker_metrics(worker).busy_duration_total.load(Relaxed);
        Duration::from_nanos(nanos)
    }

    /// TODO
    pub fn worker_local_schedule_count(&self, worker: usize) -> u64 {
        self.handle.spawner.worker_metrics(worker).local_schedule_count.load(Relaxed)
    }
    ...
    pub fn worker_overflow_count(&self, worker: usize) -> u64 {
        self.handle.spawner.worker_metrics(worker).overflow_count.load(Relaxed)
    }
}