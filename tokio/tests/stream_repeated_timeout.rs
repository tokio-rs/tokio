...
#[tokio::test]
async fn return_elapsed_errors_multiple_times() {
    time::pause();

    let stream = stream::iter(1..=3)
        .then(maybe_delay)
        .repeated_timeout(ms(50));
    let mut stream = task::spawn(stream);

    // First item completes immediately
    assert_ready_eq!(stream.poll_next(), Some(Ok(1)));

    // Second item is delayed 200ms, times out after 50ms. Multiple `Elapsed`
    // errors are returned.
    for _ in 0..3 {
        assert_pending!(stream.poll_next());
        //
