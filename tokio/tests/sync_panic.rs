...
#[test]
fn mpsc_bounded_sender_send_timeout_panic_caller() -> Result<(), Box<dyn Error>> {
    let rt = Builder::new_current_thread().build().unwrap();
    let (tx, _rx) = mpsc::channel::<u8>(1);
    rt.block_on(async {
        let _ = tx.send_timeout(3, Duration::from_millis(2)).await;
    });

    // let panic_location_file = test_panic(|| {
    //     // Runtime without time
    //     let rt = Builder::new_current_thread().build().unwrap();
    //     let (tx, _rx) = mpsc::channel::<u8>(1);
    //     rt.block_on(async {
    //         let _ = tx.send_timeout(3, Duration::from_millis(2));
    //     });
    // });
