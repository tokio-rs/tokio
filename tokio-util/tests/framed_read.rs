...
/// A test reader that returns one byte per read.
struct ByteAtATimeReader<'a>(&'a [u8]);
impl AsyncRead for ByteAtATimeReader<'_> {
    fn poll_read(
        ...
        let n = self.0.len().min(buf.remaining()).min(1);
...
#[cfg(feature = "rt")]
#[tokio::test]
async fn reading_a_line_byte_by_byte_is_cooperative() {
    use futures::StreamExt as _;

    let expected = "abcd".repeat(64);
    let input = format!("{expected}\n");
    let reader = ByteAtATimeReader(input.as_bytes());
    let mut lines = FramedRead::new(reader, tokio_util::codec::LinesCodec::new());
    let mut next_line = task::spawn(lines.next());

    tokio_test::assert_pending!(next_line.poll());
    assert_eq!(next_line.await.unwrap().unwrap(), expected);
    assert!(lines.next().await.is_none());
}

#[tokio::test]
async fn reading_a_line_byte_by_byte_remains_unconstrained() {
    use futures::StreamExt as _;
    ...
    // With coop disabled, reading the whole line must finish in a single poll.
    let line = {
        let mut next_line = task::spawn(tokio::task::unconstrained(lines.next()));
        tokio_test::assert_ready!(next_line.poll())
            .unwrap()
            .unwrap()
    };
