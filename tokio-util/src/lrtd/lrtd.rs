...
use nix::sys::signal::{self, SigAction, SaFlags, SigSet, SigHandler, Signal};
...
fn signal_all_threads(signal: Signal, targets: Vec<libc::pthread_t>) {
    for thread_id in &targets {
        let result = unsafe { libc::pthread_kill(*thread_id, match signal.into() {
...
fn probe(tokio_runtime: &Arc<Runtime>, detection_time:Duration, signal: Signal, workers: &Arc<WorkerSet>) {    
    let (tx, rx) = mpsc::channel();
    let handle = tokio_runtime.handle(); 
    let _nothing_handle = handle.spawn(do_nothing(tx));
    ...
    if !is_probe_success {
        ...
        eprintln!("Detected worker blocking, signaling worker threads: {:?}", targets);
...
impl LongRunningTaskDetector {
    ...
    pub fn new(interval: Duration, detection_time: Duration, signal: Signal, runtime_builder: &mut Builder) -> Self {
    ...
    pub fn start(&self, runtime: Arc<Runtime>)
    {
        ...
        thread::spawn(move || {
            ...
            while !*stop_flag.lock().unwrap() {
                ...
                thread::sleep(Duration::from_micros(rng.gen_range(1..=interval.as_micros()).try_into().unwrap()));
            }
        });
    }
